...
<pattern id="FI-validation-M" xmlns="http://purl.oclc.org/dsdl/schematron">
    <rule context="/*" >
        <assert id="rule|text|FI-0001" test="count(hilma:NationalExtension/hilma:ProcurementProject) or not($noticeSubType = ('E1', 'E3', 'E4'))">rule|text|FI-0001</assert>
    </rule>
    ...
    <rule context="/*/cac:TenderingTerms/cac:TendererQualificationRequest[$noticeSubType = ('14', '15', '16', '17', '19', '20', '21', '23', '24')]">
        ...
        <assert id="rule|text|FI-0002" test="empty($missing)">
            rule|text|FI-0002 [<value-of select="string-join($missing, ', ')"/>]
        </assert>
    </rule>
</pattern>